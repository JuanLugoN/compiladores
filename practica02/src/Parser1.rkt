...
(define minHS-parser
  (parser
   ...
   (precs (left ASSIGN) ; precs clause. Here we can give some precedence of our language operators.
          (left TYPEOF)
          (left OR)
          (left AND)
          (left - +)
   ...
   (grammar ; grammar clause. Here goes the grammar of minHS.
    (exp [(NUM) (num-exp $1)]
         ...
         [(IF LP exp RP THEN LK exp RK ELSE LK exp RK) (if-exp $3 $7 $11)]
...
(let ((input (open-input-string "f(#t)then{2}else{3}")))
  (minHS-parser (lex-this minHS-lexer input)))
#|
Desired response:
